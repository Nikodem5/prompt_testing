...
def main(page: ft.Page):
    ...
    def add_assistant(e):
        ...
        page.controls[2].controls.insert(0, ft.Text(f'{assistant.name} ({assistant.id})', selectable=True))
    ...
    def remove_assistant(e, assistant_id):
        ...
        i = assistant_ids.index(assistant_id)
    ...
    for assistant in assistants:
        # print(assistant.id)
        row.controls.append(ft.Column([ft.Text(f'{assistant.name} ({assistant.id})', selectable=True), ft.ElevatedButton(f"Remove {assistant.name}", on_click=lambda e: remove_assistant(e, assistant.id))]))
        # print(row.controls[-1].controls[1].on_click)
